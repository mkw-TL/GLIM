...
double glm_poisson_pl_cpp(const arma::mat &X, const arma::vec &y,
  ...
  for (int j = 0; j < m; ++j) {
    ...
    for (int i = 0; i < n; i++) {
      mle_sim += y_sim(i) * std::log(mu_hat_sim(i)) - mu_hat_sim(i) -
                 std::lgamma(y_sim(i) + 1.0);
...
#pragma omp for schedule(static)
  for (int i = 0; i < AA.size(); i++) {
    ...
    prev_xi = current_xi; // initialize our guess of the next xi to whatever was just previously seen.
  }
  return xi_mat;
}
